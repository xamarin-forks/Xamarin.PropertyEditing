		...
		: PropertyViewModel<CommonRatio>
	{
		...
		internal void ValueChanged (string stringValue, int caretPosition, int selectionLength, double incrementValue)
		{
			...
			if (selectionLength == stringValue.Length) {
				...
				// Just workout out which value the caret is closest to, so we can increment it
				var sepLocation = GetSeparatorPosition (stringValue);
				if (caretPosition <= sepLocation) {
					Value = new CommonRatio (newNumerator, Value.Denominator, separator);
				} else {
					Value = new CommonRatio (Value.Numerator, newDenominator, separator);
				}
			}
		}
		...
		internal void SetRatioFromString (string value)
		{
			bool parsed = true;
		...
		char GetSeparator (string finalString)
		{
		...
		int GetSeparatorPosition (string finalString)
		{
			return finalString.IndexOfAny (Separators);
		}
